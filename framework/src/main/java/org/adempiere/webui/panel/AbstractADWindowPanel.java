/******************************************************************************
 * Product: Posterita Ajax UI * Copyright (C) 2007 Posterita Ltd. All Rights
 ...
 * 330, Boston, MA 02111-1307 USA. * For the text or an alternative of this
 * public license, you may reach us * Posterita Ltd., 3, Draper Avenue, Quatre
 * Bornes, Mauritius * or via info@posterita.org or http://www.posterita.org/ *
 *****************************************************************************/
...
import org.adempiere.webui.apps.form.CreateFormOFCDetail;
import org.adempiere.webui.apps.form.IExceptionTips;
import org.adempiere.webui.apps.form.WCreateFromFactory;
import org.adempiere.webui.apps.form.WExceptionTips;
import org.adempiere.webui.apps.form.WOrderCompletedTips;
...
import org.compiere.model.MClassifyImport;
import org.compiere.model.MLookupFactory;
import org.compiere.model.MOWMSServiceConfig;
import org.compiere.model.MOrder;
import org.compiere.model.MProcess;
import org.compiere.model.MProductMaintenanceLine;
import org.compiere.model.MQuery;
import org.compiere.model.MReturnMaterial;
import org.compiere.model.MReturnMaterialDetail;
...
import com.winit.api.common.transceiver_3e.util.SoapUtil;
import com.winit.api.common.transceiver_3e.vo.Field;
import com.winit.api.owms.common.Constants;
import com.winit.scpTo.CopyFileToRemoteHost;
 ...
 *      https://sourceforge
 *      .net/tracker/?func=detail&atid=879335&aid=2877111&group_id=176962
 * @author hengsin, hengsin.low@idalica.com
 * @see FR [2887701]
 *      https://sourceforge.net/tracker/?func=detail&atid=879335&aid
 *      =2887701&group_id=176962
 * @sponsor www.metas.de
 * @author Teo Sarca, teo.sarca@gmail.com <li>BF [ 2992540 ] Grid/Panel not
 *         refreshed after process run
 *         https://sourceforge.net/tracker/?func=detail
 *         &aid=2992540&group_id=176962&atid=955896 <li>BF [ 2985892 ] Opening a
 *         window using a new record query is not working
 *         https://sourceforge.net
 *         /tracker/?func=detail&aid=2985892&group_id=176962&atid=955896
 */
public abstract class AbstractADWindowPanel extends AbstractUIPart implements ToolbarListener, EventListener,
		DataStatusListener, ActionListener, ASyncProcess
{
	...
	public boolean initPanel(int adWindowId, MQuery query)
	{
		...
		{
			String msg = "Timestamp=" + new Date() + ", Bug 2832968 SessionUser=" + checkad_user_id + ", ContextUser="
		...
		// End of temporary code for [ adempiere-ZK Web Client-2832968 ] User
		// context lost?
	...
	private boolean doZoomToDetail(GridTab gTab, MQuery query, int tabIndex)
	{
		...
		for (GridField field : fields)
		{
			if (field.getColumnName().equalsIgnoreCase(query.getZoomColumnName()))
			{
				...
				int parentId = DB.getSQLValue(
						null,
						"SELECT " + gTab.getLinkColumnName() + " FROM " + gTab.getTableName() + " WHERE "
					...
					while (index > 0)
					{
						...
						if (pTab.getTabLevel() < currentTab.getTabLevel())
						{
							...
							if (index > 0)
							{
								...
								{
									int pid = DB.getSQLValue(null,
											"SELECT " + pTab.getLinkColumnName() + " FROM " + pTab.getTableName()
													+ " WHERE " + currentTab.getLinkColumnName() + " = ?", oldpid);
									if (pid > 0)
									{
	...
	protected void initTab(MQuery query, int tabIndex)
	{
		...
		if (gTab.isSortTab())
		{
			...
			if (includedMap.containsKey(gTab.getAD_Tab_ID()))
			{
				includedMap.get(gTab.getAD_Tab_ID()).embed(ctx, curWindowNo, gridWindow, gTab.getAD_Tab_ID(), tabIndex,
						sortTab);
			}
			else
			{
	...
	private MQuery initialQuery(MQuery query, GridTab mTab)
	{
		...
		if (require)
		{
			...
			FindWindow find = new FindWindow(curWindowNo, mTab.getName(), mTab.getAD_Table_ID(), mTab.getTableName(),
					where.toString(), findFields, 10, mTab.getAD_Tab_ID()); // no
																			// query
																			// below
																			// 10
			if (find.getTitle() != null && find.getTitle().length() > 0)
			{
				// Title is not set when the number of rows is below the
				// minRecords parameter (10)
				if (!find.isCancel())
				{
					query = find.getQuery();
	...
	public void onAttachment()
	{
		...
		// Attachment va =
		new WAttachment(curWindowNo, curTab.getAD_AttachmentID(), curTab.getAD_Table_ID(), record_ID, null);
		...
		if (curTab != null)
		{
			...
			else if (curTab.needSave(true, false))
			{
				if (curTab.needSave(true, true))
				{
					...
					// Don't save
					{
						int newRecord = curTab.getTableModel().getNewRow(); // VOSS
																			// COM
	...
	private void updateToolbar()
	{
		...
		toolbar.enableTabNavigation(curTabIndex > 0, curTabIndex < (adTab.getTabCount() - 1));
	...
	public void dataStatusChanged(DataStatusEvent e)
	{
		...
		if (e.getAD_Message() != null || e.getInfo() != null)
		{
			...
			if (sb.length() > 0)
			{
				...
				boolean showPopup = e.isError()
						|| (!GridTab.DEFAULT_STATUS_MESSAGE.equals(e.getAD_Message()) && !GridTable.DATA_REFRESH_MESSAGE
								.equals(e.getAD_Message()));
		...
		// Check Attachment
		boolean canHaveAttachment = curTab.canHaveAttachment(); // not single
																// _ID column
		//
		if (canHaveAttachment && e.isLoading() && curTab.getCurrentRow() > e.getLoadedRows())
		{
		...
		if (e.isLoading() && curTab.getCurrentRow() > e.getLoadedRows())
		{
			canHaveChat = false;
		}
	...
	public void onFind()
	{
		...
		FindWindow find = new FindWindow(curTab.getWindowNo(), curTab.getName(), curTab.getAD_Table_ID(),
				curTab.getTableName(), curTab.getWhereExtended(), findFields, 1, curTab.getAD_Tab_ID());
	...
	public void onDeleteSelection()
	{
		...
		{
			sql = MLookupFactory.getLookup_TableDirEmbed(Env.getLanguage(ctx), keyColumnName, "[?", "?]").replace(
					"[?.?]", "?");
		}
			...
			if ("".equals(keyColumnName))
			{
				...
				for (Iterator<String> iter = parentColumnNames.iterator(); iter.hasNext();)
				{
					...
					if (field.isLookup())
					{
						...
						if (lookup != null)
						{
							displayValue = displayValue.append(lookup.getDisplay(curTab.getValue(i, columnName)))
									.append(" | ");
						}
						else
						{
	...
	public void onPrint()
	{
		...
		ProcessModalDialog dialog = new ProcessModalDialog(this, getWindowNo(), AD_Process_ID, table_ID, record_ID,
				true);
		if (dialog.isValid())
		{
	...
	public void onReport()
	{
		...
		/*
		 * // Query MQuery query = new MQuery(curTab.getTableName()); // Link
		 * for detail records String queryColumn = curTab.getLinkColumnName();
		 * // Current row otherwise if (queryColumn.length() == 0) queryColumn =
		 * curTab.getKeyColumnName(); // Find display String infoName = null;
		 * String infoDisplay = null; for (int i = 0; i <
		 * curTab.getFieldCount(); i++) { GridField field = curTab.getField(i);
		 * if (field.isKey()) infoName = field.getHeader(); if
		 * ((field.getColumnName().equals("Name") ||
		 * field.getColumnName().equals("DocumentNo") ) && field.getValue() !=
		 * null) infoDisplay = field.getValue().toString(); if (infoName != null
		 * && infoDisplay != null) break; } if (queryColumn.length() != 0) { if
		 * (queryColumn.endsWith("_ID")) query.addRestriction(queryColumn,
		 * MQuery.EQUAL, new Integer(Env.getContextAsInt(ctx, curWindowNo,
		 * queryColumn)), infoName, infoDisplay); else
		 * query.addRestriction(queryColumn, MQuery.EQUAL, Env.getContext(ctx,
		 * curWindowNo, queryColumn), infoName, infoDisplay); }
		 */

		MQuery query = curTab.getQuery();
		if (curTab.getWhereClause() != null && curTab.getWhereClause().trim().length() > 0)
		{
			query.addRestriction(curTab.getWhereClause().trim());
		}
		...
		if (queryColumn.length() != 0)
		{
			if (queryColumn.endsWith("_ID"))
				query.addRestriction(queryColumn, MQuery.EQUAL,
						new Integer(Env.getContextAsInt(ctx, curWindowNo, queryColumn)), infoName, infoDisplay);
			else
				query.addRestriction(queryColumn, MQuery.EQUAL, Env.getContext(ctx, curWindowNo, queryColumn),
	...
	public void onZoomAcross()
	{
		if (toolbar.getEvent() != null)
		{
			...
			new WZoomAcross(toolbar.getEvent().getTarget(), curTab.getTableName(), curTab.getAD_Window_ID(), query);
		}
	}
	...
	private void actionButton(WButtonEditor wButton)
	{
		...
		if (col.equals("Record_ID"))
		{
			...
			int AD_Window_ID = Env.getContextAsInt(ctx, curWindowNo, "AD_Window_ID");
			if (AD_Window_ID > 0 && AD_Table_ID > 0)
			{
				MTable table = MTable.get(ctx, AD_Table_ID);
				AEnv.zoom(AD_Window_ID, MQuery.getEqualQuery(table.getTableName() + "_ID", Record_ID));
			}
			else
			{
		...
		//
		else if (col.equals("BEComplete"))
		{
			IExceptionTips tips = ExceptionTipsFactory.create(curWindowNo, curTab, wButton);
			if (tips.isInitOK()) // may not be allowed
			{
				Window win = (Window) tips;
				win.setVisible(true);
				AEnv.showWindow(win);
			}
			else
			{
				statusBar.setStatusLine(tips.getErrorMessage(), true, true);
			}
		...
		else if (col.equals("CreateOFCDetail"))
		{
			CreateFormOFCDetail form = new CreateFormOFCDetail(curWindowNo, curTab, wButton);
			if (form.isInitOK()) // may not be allowed
			{
				Window win = (Window) form;
				win.setVisible(true);
				AEnv.showWindow(win);
			}
			else
			{
				statusBar.setStatusLine(form.getErrorMessage(), true, true);
			}

			onRefresh();
		}

		else if (col.equals("UploadPic"))
		{
			UploadPic(record_ID);
			onRefresh(false);
		} // UploadPic

		else if (col.equals("CopyTaxRule"))
		{
			MProductMaintenanceLine thisMaintenance = new MProductMaintenanceLine(this.ctx, record_ID, null);

			List<MClassifyImport> list = thisMaintenance.getChildren(MClassifyImport.Table_Name, null, null);

			if (list.size() <= 0)
			{
				// TODO 不能复制没有维护的产品归类信息
				throw new AdempiereException("@WT_Copy_Forbiden@");
			}

			Env.setContext(this.ctx, "#WT_ProductMaintenanceLine_RecordID", record_ID);

			thisMaintenance.saveEx();

			onRefresh();
		} // CopyTaxRule

		else if (col.equals("PasteTaxRule"))
		{
			int recordID = Env.getContextAsInt(this.ctx, "#WT_ProductMaintenanceLine_RecordID");

			if (recordID == 0)
			{
				// TODO 无效操作 ： 您没有选择需要复制的内容！
				throw new AdempiereException("@WT_Paste_Forbiden@");
			}

			MProductMaintenanceLine copyMaintainceLine = new MProductMaintenanceLine(this.ctx, recordID, null);

			MProductMaintenanceLine pasteMaintainceLine = new MProductMaintenanceLine(this.ctx, record_ID, null);

			List<MClassifyImport> list = copyMaintainceLine.getChildren(MClassifyImport.Table_Name, null, null);

			List<MClassifyImport> thisList = pasteMaintainceLine.getChildren(MClassifyImport.Table_Name, null, null);

			// Copy Product Classify Import Message
			if (thisList.size() > 0)
			{
				for (MClassifyImport classfiy : list)
				{
					for (MClassifyImport thisClassfiy : thisList)
					{
						if (thisClassfiy.getC_Country_ID() == classfiy.getC_Country_ID())
						{
							thisClassfiy.setWT_CustomsTaxRule_ID(classfiy.getWT_CustomsTaxRule_ID());
							thisClassfiy.setDeclareName(classfiy.getDeclareName());
							thisClassfiy.saveEx();
						}
					}
				}
		...
		else if (col.equals("LookPic"))
		{
			MTable table = MTable.get(ctx, table_ID);
			PO model = table.getPO(record_ID, null);
			String imageURL = (String) model.get_Value("ImageURL");
			if (imageURL != null) {
				try {
					String imageIP = getIPByReturnDetailId(record_ID);
					if (StringUtils.isNotEmpty(imageIP)) {
						imageURL = "IP" + File.separator
								+ URLEncoder.encode(imageIP, "UTF-8")
			...
			Env.startBrowser(MSysConfig.getValue("WT_ShowImage") + imageURL);
		} // LookPic
		...
		{
			ProcessModalDialog dialog = new ProcessModalDialog(this, curWindowNo, wButton.getProcess_ID(), table_ID,
	...
	private static class ExceptionTipsFactory
	{
		static IExceptionTips create(int curWindowNo, final GridTab curTab, final WButtonEditor wButton)
		{
			IExceptionTips retValue = null;
			if (curTab.getAD_Table_ID() == MTTaskGroup.Table_ID)
			{
				retValue = new WExceptionTips(curWindowNo, curTab, wButton);
			}
			else if (curTab.getAD_Table_ID() == MOrder.Table_ID || curTab.getAD_Table_ID() == MWorkOrder.Table_ID)
			{
				retValue = new WOrderCompletedTips(curWindowNo, curTab, wButton);
			}
			return retValue;
		}
	}
	...
	public void unlockUI(ProcessInfo pi)
	{
		...
		m_uiLocked = false;
		boolean notPrint = pi != null && pi.getAD_Process_ID() != curTab.getAD_Process_ID()
	...
	private void updateUI(ProcessInfo pi)
	{
		...
		if (logInfo.length() > 0)
			FDialog.info(curWindowNo, this.getComponent(), Env.getHeader(ctx, curWindowNo), pi.getTitle() + "<br>"
					+ logInfo);
	}
	 ...
	 * UploadPic
	 */
	private void UploadPic(int record_ID)
	{
		Trx trx = Trx.get("UpdateRMDetailUploadPic", true);
		MReturnMaterialDetail rmd = new MReturnMaterialDetail(this.ctx, record_ID, trx.getTrxName());
		//
		String user = MSysConfig.getValue("WT_ImageServerUser");
		String passwd = MSysConfig.getValue("WT_ImageServerPasswd");
		String host = MSysConfig.getValue("WT_ImageServerHost");
		// 单个图片最大字节
		int maxSize = MSysConfig.getIntValue("WT_MaximalImageSize", 1024);
		Object[] args = { maxSize };
		// 图片服务器存放绝对路径 /home/Data/Pictures/
		String imageServerPath = MSysConfig.getValue("WT_ImageServer");
		imageServerPath = imageServerPath + rmd.getC_BPartner_ID() + File.separator
				+ rmd.getWT_ReturnMaterialDetail_ID() + File.separator;
		// 在图片服务器上创建相关目录
		try
		{
			CopyFileToRemoteHost.mkdir(user, passwd, host, imageServerPath);
		}
		...
		// 图片存放在本地的临时路径 /tmp/
		String imageTmpPath = MSysConfig.getValue("WT_ImageTMP");
		imageTmpPath = imageTmpPath + rmd.getC_BPartner_ID() + File.separator + rmd.getWT_ReturnMaterialDetail_ID()
				+ File.separator;
		// 图片显示网址
		// Value/M01/ItemSerNo/S010000000000034/Name/手机/EName/iphone/path_1/1000000/path_2/1000002/images/WT_6145406238699566043_6.jpg,WT_74804676571773811_5.jpg
		// String imageURL = MSysConfig.getValue("WT_ShowImage");
		String imageURL = StringUtils.trimToEmpty(rmd.getImageURL());
		// 图片在服务器中的相对路径 /1000000/1000000/1.jpg,2.jpg
		String filePath = StringUtils.trimToEmpty(rmd.getFilePath());
		// Show File Open Dialog
		Media[] file = null;
		try
		{
			file = Fileupload.get(Msg.getMsg(ctx, "WTP_ImageMaximalSize", args), null, -1, maxSize, true);
			if (file == null)
				return;
		}
		catch (InterruptedException e)
		...
		try
		{
			for (int i = 0; i < len; i++)
			{
				File f = new File(imageTmpPath);
				if (!f.exists())
					f.mkdirs();
				File tempFile = File.createTempFile("WT_", "_" + file[i].getName(), f);
				if (i == 0 && (rmd.getImageURL() == null || rmd.getImageURL().trim().length() == 0))
				{
					imageURL = imageURL + "Value" + File.separator + rmd.getM_Product().getValue() + File.separator
							+ "ItemSerNo" + File.separator + rmd.getItemSerNo() + File.separator + "Name"
							+ File.separator + URLEncoder.encode(rmd.getM_Product().getName(), "UTF-8") + File.separator + "EName" + File.separator
							+ URLEncoder.encode(rmd.getM_Product().getEName(), "UTF-8") + File.separator + "path_1" + File.separator
							+ rmd.getC_BPartner_ID() + File.separator + "path_2" + File.separator
							+ rmd.getWT_ReturnMaterialDetail_ID() + File.separator + "images" + File.separator
							+ tempFile.getName();
					filePath = File.separator + rmd.getC_BPartner_ID() + File.separator
							+ rmd.getWT_ReturnMaterialDetail_ID() + File.separator + tempFile.getName();
				}
				else
				{
					imageURL = imageURL + "," + tempFile.getName();
				...
				if (file[i].inMemory())
				{
					bytes = file[i].getByteData();
				}
				else
				{
					InputStream is = file[i].getStreamData();
				...
				fos.close();
				// 将图片传到图片服务器对应目录
				CopyFileToRemoteHost.scpto(user, passwd, host, imageServerPath, imageTmpPath + File.separator
						+ tempFile.getName(), tempFile.getName());
			}
			rmd.setImageURL(imageURL);
			rmd.setFilePath(filePath);
			rmd.setCounter(len + rmd.getCounter());
			rmd.saveEx();
			FDialog.info(curWindowNo, parent, Msg.getMsg(ctx, "Upload Success"));
			// 
			callWinitErp(rmd);
			trx.commit();
		}
	 ...
	 * 调用ERP	 
	 */
	private void callWinitErp(MReturnMaterialDetail rmd)
	{
		List<Field> fieldList = new ArrayList<Field>();
		fieldList.add(new Field("ItemSerNo", rmd.getItemSerNo()));
		fieldList.add(new Field("RM_NO",  rmd.getWT_ReturnMaterial().getDocumentNo()));
		fieldList.add(new Field("Counter", rmd.getCounter()+""));
		fieldList.add(new Field("FilePath", rmd.getFilePath()));
		fieldList.add(new Field("ImageURL", rmd.getImageURL()));
		SoapUtil.sendWebService(fieldList, Constants.WebServiceType.ReturnMaterial.DetailUploadPic);
	}
	
	private String getIPByReturnDetailId(int detailId) throws SQLException {
		/**
		 * SELECT SC.M_WAREHOUSE_ID FROM WT_OWMS_SERVICECONFIG SC,
		 * WT_RETURNMATERIAL RM, WT_RETURNMATERIALDETAIL RMD WHERE
		 * SC.M_WAREHOUSE_ID = RM.M_WAREHOUSE_ID AND RM.WT_RETURNMATERIAL_ID =
		 * RMD.WT_RETURNMATERIAL_ID AND RMD.WT_RETURNMATERIALDETAIL_ID = 1003271
		 */
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT SC.").append(
				MOWMSServiceConfig.COLUMNNAME_imageIP).append(" ip");
		sql.append("  FROM ").append(MOWMSServiceConfig.Table_Name)
				.append(" SC, ");
		sql.append(MReturnMaterial.Table_Name).append(" RM, ");
		sql.append(MReturnMaterialDetail.Table_Name).append(" RMD ");
		sql.append(" WHERE SC.")
				.append(MOWMSServiceConfig.COLUMNNAME_M_Warehouse_ID)
				.append(" = RM.")
				.append(MReturnMaterial.COLUMNNAME_M_Warehouse_ID);
		sql.append("   AND RM.")
				.append(MReturnMaterial.COLUMNNAME_WT_ReturnMaterial_ID)
				.append(" = RMD.")
				.append(MReturnMaterialDetail.COLUMNNAME_WT_ReturnMaterial_ID);
		sql.append("   AND RMD.")
				.append(MReturnMaterialDetail.COLUMNNAME_WT_ReturnMaterialDetail_ID)
				.append(" = ").append(detailId);

		CPreparedStatement prst = null;
		ResultSet rs = null;
		try {
			prst = DB.prepareStatement(sql.toString(), null);
