...
/**
 * Replacement for the Swing client minigrid component ZK Listbox extension for
 * Adempiere Web UI. The listbox contains a model and a renderer. The model
 * holds the underlying data objects, while the renderer deals with displaying
 * the data objects. The renderer will render data objects using a variety of
 * components. These components can then be edited if they are not readonly.
 * 
...
public class WListbox extends Listbox implements IMiniTable, TableValueChangeListener, WTableModelListener
{
	...
	public void setModel(ListModel model)
	{
		...
		{
			// TODO need to remove listener before adding, but how to do this
			// without
	...
	{
		// if the first column holds a boolean and it is false, it is not
		// editable
	 ...
	 * <p>
	 * This is unused for this implementation because the column ordering cannot
	...
	/**
	 * Set Column at the specified <code>index</code> to read-only or
	 * read/write.
	 * 
	 * @param index index of column to set as read-only (or not)
	 * @param readOnly Read only value. If <code>true</code> column is read
	 *            only, if <code>false</code> column is read-write
	 */
			...
			String tableName, boolean addAccessSQL)
	{
		...
		for (columnIndex = 0; columnIndex < layout.length; columnIndex++)
		{
			setColumnClass(columnIndex, layout[columnIndex].getColClass(), layout[columnIndex].isReadOnly(),
	 ...
	 * 
	 * @param rs ResultSet containing data t enter int the table. The contents
	 *            must conform to the column layout defined in
		...
		try
		{
			while (rs.next())
			{
				...
				for (col = 0; col < m_layout.length; col++)
				{
					...
					{
						throw new ApplicationException("Cannot enter a " + columnClass.getName() + " in column " + col
	...
	/**
	 * Load Table from Vector
	 * 
	 * @param tableData
	 */
	public void loadTable(Vector<Vector> tableData)
	{
		int row = 0; // model row
		int col = 0; // model column
		Object data = null;
		...
		for (Vector vData : tableData)
		{
			...
			for (col = 0; col < m_layout.length; col++)
			{
				...
				if (columnClass == IDColumn.class)
				{
					data = new IDColumn((Integer) vData.get(rsColIndex - 1));
				}
				else if (columnClass == Boolean.class)
				{
					data = (Boolean) vData.get(rsColIndex - 1);
				}
				else if (columnClass == Timestamp.class)
				{
					data = (Timestamp) vData.get(rsColIndex - 1);
				}
				else if (columnClass == BigDecimal.class)
				{
					data = (BigDecimal) vData.get(rsColIndex - 1);
				}
				else if (columnClass == Double.class)
				{
					data = (Double) vData.get(rsColIndex - 1);
				}
				else if (columnClass == Integer.class)
				{
					data = (Integer) vData.get(rsColIndex - 1);
				}
				else
				{
					// TODO factor out this cleanup
					String s = vData.get(rsColIndex - 1).toString();
					if (s != null)
					{
						data = s.trim(); // problems with NCHAR
					}
	...
	{
		int col = event.getColumn(); // column of table field which caused the
										// event
	...
	/**
	 * Respond to a change in the table's model. If the event indicates that the
	 * entire table has changed, the table is repainted.
	 * 
	...
	public void tableChanged(WTableModelEvent event)
	{
				...
				&& (event.getFirstRow() == WTableModelEvent.ALL_ROWS))
		{
			this.repaint();
				...
				&& event.getFirstRow() != WTableModelEvent.ALL_ROWS && !m_readWriteColumn.isEmpty())
		{
